...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
	...
	public function widthForStringUsingFontSize2($string, $font, $fontSize)
	{
		try
		{
	...
	public function WrapTextToWidth($page, $text, $width)
	{
		...
		for($i = 0;$i<count($t_words);$i++)
		{
			...
			if ($this->widthForStringUsingFontSize2($current_line.' '.$t_words[$i], $page->getFont(), $page->getFontSize()) < $width)
	...
	public function drawTextInBlock(&$page, $text, $x, $y, $width, $height, $alignment = 'c', $encoding = 'UTF-8')
	{
		...
		$text_width = $this->widthForStringUsingFontSize2($text, $page->getFont(), $page->getFontSize());
		switch ($alignment)
		{
